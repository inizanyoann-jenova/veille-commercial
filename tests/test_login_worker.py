...
def _run_worker(page, url, selectors, email="u@u.com", password="pass"):
    ...
    with patch("playwright.sync_api.sync_playwright") as pw_mock:
        browser = (
            pw_mock.return_value.__enter__.return_value.chromium.launch.return_value
        )
...
def test_worker_uses_wait_for_selector_not_query_selector():
    ...
    assert (
        "query_selector"
        not in src.split("wait_for_selector")[0].split("def test_login")[1]
...
def test_worker_succeeds_when_field_appears_after_delay():
    ...
    with patch("playwright.sync_api.sync_playwright") as pw_mock:
        ...
        page.wait_for_selector.side_effect = lambda sel, **kw: (
            el_email if "email" in sel else el_pwd
        )
