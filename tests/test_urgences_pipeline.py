...
def _add_tender(
    db, id, title, score, deadline_offset_days, status="À qualifier", blacklisted=False
):
    from models import Tender

    midnight = _today_midnight()
    deadline = (
        (midnight + timedelta(days=deadline_offset_days))
...
def test_urgences_excludes_gagne_perdu(db):
    ...
    _add_tender(
        db, "u5", "SSI Mairie", score=80, deadline_offset_days=5, status="Gagné"
    )
    _add_tender(
        db, "u6", "CMSI Centre", score=80, deadline_offset_days=5, status="Perdu"
    )
...
def test_urgences_excludes_blacklisted(db):
    ...
    _add_tender(
        db, "u7", "SSI Stade", score=80, deadline_offset_days=5, blacklisted=True
    )
...
def test_pipeline_soumis_column(db):
    ...
    _add_tender(
        db, "p3", "CMSI Hôpital", score=80, deadline_offset_days=10, status="Soumis"
    )
...
def test_pipeline_resultats_column(db):
    ...
    _add_tender(
        db, "p4", "SSI Gagné", score=80, deadline_offset_days=10, status="Gagné"
    )
    _add_tender(
        db, "p5", "SSI Perdu", score=80, deadline_offset_days=10, status="Perdu"
    )
