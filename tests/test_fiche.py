...
def test_go_deadline_passe():
    d = _compute_fiche_data(
        70, -3, "🔥 SSI / Détection incendie", "🏝️ La Réunion", False, "", {}
    )
...
def test_go_delai_critique():
    d = _compute_fiche_data(
        70, 5, "🔥 SSI / Détection incendie", "🏝️ La Réunion", False, "", {}
    )
...
def test_go_priorite():
    d = _compute_fiche_data(
        70, 20, "🔥 SSI / Détection incendie", "🏝️ La Réunion", False, "", {}
    )
...
def test_go_planifier():
    d = _compute_fiche_data(
        70, None, "🔥 SSI / Détection incendie", "🏝️ La Réunion", False, "", {}
    )
...
def test_sm_ssi():
    d = _compute_fiche_data(
        70, None, "🔥 SSI / Détection incendie", "Non précisé", False, "", {}
    )
    assert d["sm"] == 45
...
def test_sm_cmsi():
    d = _compute_fiche_data(
        70, None, "💨 CMSI / Désenfumage", "Non précisé", False, "", {}
    )
    assert d["sm"] == 40
...
def test_sm_video():
    d = _compute_fiche_data(
        70, None, "📷 Vidéosurveillance / CCTV", "Non précisé", False, "", {}
    )
    assert d["sm"] == 40
...
def test_sm_courants():
    d = _compute_fiche_data(
        70, None, "⚡ Courants faibles", "Non précisé", False, "", {}
    )
    assert d["sm"] == 30
...
def test_sk_one_keyword_gives_6():
    d = _compute_fiche_data(
        70, None, "Autre", "Non précisé", False, "Installation SSI bâtiment", {}
    )
    assert d["sk"] == 6
...
def test_sk_two_keywords_gives_10():
    d = _compute_fiche_data(
        70, None, "Autre", "Non précisé", False, "Marché CMSI désenfumage", {}
    )
    assert d["sk"] == 10
...
def test_sk_three_keywords_gives_15():
    d = _compute_fiche_data(
        70, None, "Autre", "Non précisé", False, "SSI CMSI détection incendie", {}
    )
    assert d["sk"] == 15
...
def test_sk_absent():
    d = _compute_fiche_data(
        70, None, "Autre", "Non précisé", False, "Nettoyage parkings", {}
    )
    assert d["sk"] == 0
...
def test_score_exactement_go():
    d = _compute_fiche_data(
        65, None, "🔥 SSI / Détection incendie", "🏝️ La Réunion", False, "", {}
    )
    assert d["label_action"] == "🟢 Planifier la réponse"
...
def test_jours_restants_zero_label():
    d = _compute_fiche_data(
        70, 0, "🔥 SSI / Détection incendie", "🏝️ La Réunion", False, "", {}
    )
    assert d["label_action"] == "🚨 Action immédiate — délai critique"
...
def test_atout_ocean_indien():
    d = _compute_fiche_data(
        70, None, "🔥 SSI / Détection incendie", "🌍 Madagascar", False, "", {}
    )
...
def test_atout_maintenance():
    d = _compute_fiche_data(
        70, None, "🔥 SSI / Détection incendie", "Non précisé", True, "", {}
    )
...
def test_risque_concurrent():
    ...
    d = _compute_fiche_data(
        70, 30, "🔥 SSI / Détection incendie", "🏝️ La Réunion", False, "", a
    )
...
def test_risque_delai_court():
    d = _compute_fiche_data(
        70, 7, "🔥 SSI / Détection incendie", "🏝️ La Réunion", False, "", {}
    )
    assert any("Délai très court" in r for r in d["risques"])
...
def test_risque_vide_si_tout_ok():
    d = _compute_fiche_data(
        70, 60, "🔥 SSI / Détection incendie", "🏝️ La Réunion", False, "", {}
    )
    assert d["risques"] == []
...
def test_score_above_100_clamped():
    d = _compute_fiche_data(
        150, None, "🔥 SSI / Détection incendie", "🏝️ La Réunion", False, "", {}
    )
    assert d["label_action"] == "🟢 Planifier la réponse"
...
def test_none_a_dict_defaults_gracefully():
    d = _compute_fiche_data(
        70, 30, "🔥 SSI / Détection incendie", "🏝️ La Réunion", False, "", None
    )
...
def test_get_acheteur_history_finds_similar_tenders(db, make_tender):
    ...
    make_tender(
        title="Installation detection incendie lycee", status="Gagne", amount=80000
    )
...
def test_get_acheteur_history_excludes_self(db, make_tender):
    ...
    make_tender(
        title="Installation detection incendie lycee", status="Gagne", amount=50000
    )
    result = get_acheteur_history(db, t)
