...
def test_is_construction_relevant_hotel():
    assert (
        is_construction_relevant("Construction d'un hôtel 4 étoiles à Saint-Denis")
        is True
    )
...
def test_prive_ssi_direct():
    ...
    assert (
        is_prive_relevant("Installation d'un SSI dans un bâtiment commercial") is True
    )
...
def test_prive_construction_hopital():
    ...
    assert (
        is_prive_relevant("Lancement du chantier du nouvel hôpital de Saint-Pierre")
        is True
    )
...
def test_prive_renovation_lycee():
    ...
    assert (
        is_prive_relevant(
            "Réhabilitation du lycée de Saint-Paul : travaux prévus en 2025"
        )
        is True
    )
...
def test_prive_nouveau_hotel():
    ...
    assert (
        is_prive_relevant("Construction d'un nouveau resort 5 étoiles à Grand Baie")
        is True
    )
...
def test_prive_livres_scolaires_rejete():
    ...
    assert (
        is_prive_relevant("La Région distribue des livres scolaires aux élèves de 6e")
        is False
    )
...
def test_prive_ecole_seule_rejetee():
    ...
    assert (
        is_prive_relevant(
            "L'école de Saint-Denis accueille de nouveaux élèves cette année"
        )
        is False
    )
...
def test_prive_hopital_actualite_rejetee():
    ...
    assert (
        is_prive_relevant(
            "Le CHU de La Réunion recrute des infirmiers pour son service urgences"
        )
        is False
    )
...
def test_prive_rentre_scolaire_rejetee():
    ...
    assert (
        is_prive_relevant(
            "Rentrée scolaire 2024 : tout ce qu'il faut savoir pour les lycéens"
        )
        is False
    )
...
def test_search_query_source():
    rows = [
        _make_row("Marché 1", source="boamp.fr"),
        _make_row("Marché 2", source="ted.europa.eu"),
    ]
    q = "boamp"
    result = [r for r in rows if q in r["Titre"].lower() or q in r["Source"].lower()]
    assert len(result) == 1
...
def test_search_query_empty_returns_all():
    ...
    result = (
        rows
        if not q
        else [r for r in rows if q in r["Titre"].lower() or q in r["Source"].lower()]
    )
    assert len(result) == 2
...
def test_classify_robinet_incendie_arme():
    ok, tags = classify_relevance(
        "Remplacement des robinets incendie armés du bâtiment A"
    )
    assert ok is True
...
def test_classify_bloc_autonome_alarme():
    ok, tags = classify_relevance(
        "Fourniture de blocs autonomes alarme sonores et lumineux"
    )
    assert ok is True
...
def test_classify_mise_en_conformite():
    ok, tags = classify_relevance(
        "Mise en conformité des installations de sécurité ERP"
    )
    assert ok is True
...
def test_classify_verification_reglementaire():
    ok, tags = classify_relevance(
        "Vérification réglementaire des équipements de sécurité"
    )
    assert ok is True
...
def test_classify_verification_periodique():
    ok, tags = classify_relevance(
        "Contrat de vérification périodique des extincteurs et RIA"
    )
    assert ok is True
...
def test_classify_gtb_direct():
    ok, tags = classify_relevance(
        "Mise en service GTB du nouveau bâtiment administratif"
    )
    assert ok is True
...
def test_classify_bms_direct():
    ok, tags = classify_relevance(
        "Installation BMS pour la gestion technique centralisée"
    )
    assert ok is True
...
def test_classify_gestion_technique_batiment():
    ok, tags = classify_relevance(
        "Marché de gestion technique bâtiment — lycée Saint-Paul"
    )
    assert ok is True
...
def test_classify_contrat_maintenance_ssi():
    ok, tags = classify_relevance(
        "Contrat de maintenance SSI EHPAD Saint-Pierre La Réunion"
    )
    assert ok is True
