...
def test_save_credential_known_site():
    with patch.object(_m, "_CredMgr") as mock_cm:
        resp = _client.post(
            "/api/credentials/nukema", json={"email": "u@u.com", "password": "secret"}
        )
    assert resp.status_code == 200
...
def test_save_credential_unknown_site():
    resp = _client.post(
        "/api/credentials/nonexistent_xyz", json={"email": "u@u.com", "password": "p"}
    )
...
def test_test_credential_playwright_wrong_password():
    ...
    mock_proc = MagicMock()
    mock_proc.stdout = _json_mod.dumps(
        {"ok": False, "erreur_page": "Mot de passe erroné"}
    )
    with patch.object(_m, "_subprocess") as mock_sub:
        mock_sub.run.return_value = mock_proc
        mock_sub.TimeoutExpired = TimeoutError
...
def test_test_credential_playwright_timeout():
    with patch.object(_m, "_subprocess") as mock_sub:
        ...
        mock_sub.TimeoutExpired = TimeoutError
        resp = _client.post(
            "/api/credentials/nukema/test", json={"email": "u@u.com", "password": "p"}
        )
    assert resp.status_code == 200
    body = resp.json()
    assert body["ok"] is False
...
def test_test_credential_unknown_site_returns_404():
    resp = _client.post(
        "/api/credentials/nonexistent_xyz/test",
