...
def test_marchessecurises_missing_credentials():
    ...
    db = _make_db()
    with (
        patch("scraper_marchessecurises.CredentialManager") as mock_cm,
    ...
    assert (
        call_kwargs.kwargs.get("error") == "CREDENTIALS_MISSING"
        or (len(call_kwargs.args) > 4 and call_kwargs.args[4] == "CREDENTIALS_MISSING")
        or "CREDENTIALS_MISSING" in str(call_kwargs)
    )
...
def test_instao_missing_credentials():
    ...
    db = _make_db()
    with (
        patch("scraper_instao.CredentialManager") as mock_cm,
...
def test_tendersgo_missing_credentials():
    ...
    db = _make_db()
    with (
        patch("scraper_tendersgo.CredentialManager") as mock_cm,
